...
class Camera {
public:
  Mat4 getView(void) const { return MathUtils::lookAt(position, target, up); }
  Mat4 getProjection(void) const {
    return MathUtils::perspective(MathUtils::radians(fov), aspectRatio,
                                  nearPlane, farPlane);
  }
...
private:
  ...
  float yaw =
      MathUtils::degrees(atan2((position - target).z, (position - target).x));
