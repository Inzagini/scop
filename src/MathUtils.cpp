...
float MathUtils::radians(const float degree) { return degree * (M_PI / 180); }

float MathUtils::degrees(const float rad) { return rad * (180 / M_PI); }
...
Mat4 MathUtils::multiply(const Mat4& a, const Mat4& b) {
  ...
  for (int c = 0; c < 4; ++c) {
    for (int r_ = 0; r_ < 4; ++r_) {
      r.m[c][r_] = a.m[0][r_] * b.m[c][0] + a.m[1][r_] * b.m[c][1] +
                   a.m[2][r_] * b.m[c][2] + a.m[3][r_] * b.m[c][3];
    }
  }
  return r;
}
...
Mat4 MathUtils::perspective(const float& fov, const float& aspect,
